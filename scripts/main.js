    ...
    db.collection(collection).orderBy("length").get()   //the collection called "hikes"
        .then(allHikes => {
            ...
            allHikes.forEach(doc => { //iterate thru each doc
                ...
                newcard.querySelector('i').id = 'save-' + docID; // for assigning unique to each hike page
...
function updateBookmark(hikeID) {
    currentUser.get().then(userDoc => {
        ...
        let isBookmarked = bookmarks.includes(hikeID); //check if it's in database

        if (isBookmarked) {
            currentUser.update({
                bookmarks: firebase.firestore.FieldValue.arrayRemove(hikeID) //update backend
            }).then(() => {
                console.log("removed")
                document.getElementById(iconID).innerText = 'bookmark_border' //update frontend
            })
        } else {
            currentUser.update({
